...
# Helper functions
def calculate_risk_score(attendance_percentage: float, recent_trend: List[Dict]) -> float:
    """Calculate risk score based on attendance and trends"""
    base_risk = max(0, 100 - attendance_percentage)
    
    # Analyze recent trend
    if len(recent_trend) >= 5:
        recent_attendance = sum(1 for a in recent_trend[-5:] if a['status'] == 'present') / 5
        trend_multiplier = 1.0 - (recent_attendance - 0.5) * 0.5  # Adjust risk based on recent trend
        base_risk *= trend_multiplier
...
def generate_recommendations(risk_level: str, attendance_percentage: float) -> List[str]:
    """Generate personalized recommendations"""
    recommendations = []
    
    if risk_level == "high":
        recommendations.extend([
            "Immediate intervention required - attendance below acceptable levels",
        ...
        recommendations.extend([
            ...
            "Consider leadership roles in class activities",
...
def predict_dropout_probability(risk_score: float, attendance_percentage: float, grades: float) -> float:
    """Predict dropout probability using simple heuristics"""
    # Simple heuristic model (in production, use trained ML model)
    attendance_factor = (100 - attendance_percentage) / 100
    grade_factor = max(0, (60 - grades) / 100) if grades > 0 else 0.5
    risk_factor = risk_score / 100
    
    dropout_prob = (attendance_factor * 0.4 + grade_factor * 0.3 + risk_factor * 0.3) * 100
    return min(100, max(0, dropout_prob))

def predict_performance_trend(attendance_percentage: float, recent_attendance: List[Dict]) -> float:
    """Predict future performance based on attendance trends"""
    if len(recent_attendance) < 3:
        return attendance_percentage
    
    # Calculate recent trend
    recent_present = sum(1 for a in recent_attendance[-5:] if a['status'] == 'present')
    recent_rate = recent_present / min(len(recent_attendance), 5)
    
    # Weight current and recent performance
    predicted_performance = (attendance_percentage * 0.6 + recent_rate * 100 * 0.4)
    return min(100, max(0, predicted_performance))
...
def generate_fallback_response(message: str, context: Dict) -> str:
    ...
    if "attendance" in message_lower:
        return f"Your current attendance is {context['attendance']['percentage']:.2f}% ({context['attendance']['presentClasses']}/{context['attendance']['totalClasses']} classes). {'Keep up the good work!' if context['attendance']['percentage'] >= 75 else 'Try to improve your attendance for better academic performance.'}"
        ...
        return f"Your current risk level is {context['performance']['riskLevel']}. {risk_advice.get(context['performance']['riskLevel'], 'Focus on consistent attendance and study habits.')}"
    
    elif "help" in message_lower or "recommendation" in message_lower:
        return context['performance']['recommendations'][0] if context['performance']['recommendations'] else "Keep maintaining good attendance and study habits. Don't hesitate to ask for help when you need it."
